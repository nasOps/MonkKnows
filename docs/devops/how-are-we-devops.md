...
Da vi startede med en feature/development branch-strategi, havde vi en tendens til ikke at slette branches af frygt for at miste noget, hvis der fejlede. I takt med at vi overgik til trunk-based og vores workflows blev mere robuste, forsvandt behovet for at holde branches i live.
...
Vi har bygget en pipeline der fanger fejl tidligt, så vi ikke opdager dem i produktion. RuboCop til linting kører både lokalt via pre-commit hooks og i CI'en. Brakeman til statisk sikkerhedsanalyse af Ruby-kode kører i CI'en. Bundler Audit og Trivy til sikkerhedssårbarheder i dependencies og images, Hadolint til Dockerfile-analyse kører ligeledes i CI'en. OWASP ZAP kører i vores CF-workflow (Continuous Feedback) op mod en lokal instans af applikationen. Smoke tests kører i CI mod en lokal Docker-instans og i CD mod monkknows.dk som post-deploy verifikation. Unit tests og integrationstests er skrevet med RSpec, og vi har desuden en Playwright e2e-test, som alle indgår i pipelines. De øvrige tools kunne i princippet også sættes op som pre-commit hooks lokalt, men det har vi valgt ikke at gøre. CodeRabbit er desuden integreret som AI-drevet code review direkte i PRs.

Tilsammen betyder det at vi fejler hurtigt og dermed kan recovere hurtigt.

---

## Uge 6 - The A in CALMS: Automate Everything

Vi har automatiseret en bred vifte af processer for at fjerne friktion og gøre arbejdet lettere. PR-merges trigger automatisk lukning og sletning af branches. Issues rykker automatisk til "Done" i vores kanban-board. Vi har en fuld CI/CD/CD/CF-pipeline. Pre-commit hooks kører Rubocop inden et commit kan laves. Postman er sat op med en collection der tester vores API-endpoints og HTML-sider, herunder register, login, logout, search og weather. Hvert endpoint testes for svartid, korrekt statuskode, Content-Type og skemavalidering af response-strukturen. Det samlede resultat er at manuelle og gentagne opgaver i høj grad er taget ud af ligningen.

---

## Uge 8 - End-to-End Responsibility

End-to-End Responsibility dækkes i høj grad af det vi allerede har beskrevet under Shared Responsibility og Collaboration. Alle tre gruppemedlemmer har haft en aktiv rolle i både udvikling og drift, og ingen har haft eneansvar for hverken Dev eller Ops. Strukturen i projektet har gjort det muligt at løfte på tværs.
...
På applikationsniveau har vi håndteret SQL injection ved at bruge parameteriserede queries, hvilket er et eksempel på at tænke sikkerhed ind i designet. Men samlet set er det et område, hvor vi har lært at sikkerhed skal med fra dag ét og ikke hægtes på til sidst.

---

## Uge 10 - Value-Stream Mapping og LEAN

På det stadie vi er nået til nu, med vores workflows, monitorering og diverse alerts, har det ikke været en udfordring at *finde* bottlenecks, da de bliver fanget automatisk. Udfordringen har i stedet været at løse dem løbende når de opstår.
...
**Culture**
Vi har løftet projektet godt. Fremdriften har været der, arbejdsprocessen har fungeret, og vi har haft en åben dialog. Det vi ikke har været gode nok til er at løfte hinanden, når det har været nødvendigt. Når nogen har haft det svært eller været fraværende, har vi ikke i tilstrækkelig grad trådt til som gruppe.

Vi vidste fra dag ét at DevOps er en kultur, det blev sagt tidligt i kurset, og vi nikkede alle til det. Men det var noget vi forstod intellektuelt, ikke noget vi reelt havde mærket. Først mod slutningen af projektet oplevede vi på egen krop hvad det egentlig betyder, nemlig at det ikke er nok at have de rigtige processer og workflows på plads, hvis man ikke aktivt løfter hinanden som mennesker. Det er den læring der sidder tilbage.
...
**Sharing**
Vi har haft siloer, og PRs har til tider ligget længe uden review, hvilket har blokeret videndeling og fremdrift. Det strider mod princippet om at dele åbent og hurtigt.
